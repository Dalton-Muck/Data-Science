...
for i in range(len(hist1_np_data)): #rows
    ...
    for j in range(len(hist1_np_data)): #columns
        ...
        if(M11 == 0):
            ...
            Distance.append(1 - (M11 / (M11 + M10 + M01)))
...
# Jaccard Heatmap
# Convert Jaccard list to a 2D numpy array
size = len(hist1_np_data)
Jaccard_matrix = np.array(Jaccard).reshape(size, size)

# Create a heatmap using seaborn
plt.figure(figsize=(10, 8))
sns.heatmap(Jaccard_matrix, cmap="coolwarm")
plt.title("Jaccard Similarity Heatmap for Hist1 Region")
plt.show()

# Distance Heatmap
# Convert Distance list to a 2D numpy array
Distance_matrix = np.array(Distance).reshape(size, size)

# Create a heatmap using seaborn
plt.figure(figsize=(10, 8))
sns.heatmap(Distance_matrix, cmap="coolwarm")
plt.title("Jaccard Distance Heatmap for Hist1 Region")
plt.show()
